...
export function PharmacistNotificationsView() {
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      setError(null);
      const res = await fetch('/api/pharmacist/notifications');
  ...
  const handleMarkAllRead = async () => {
    ...
    setMarkingAll(true);
    try {
      const res = await fetch('/api/pharmacist/notifications/read-all', { method: 'PUT' });
  ...
  const handleNotificationClick = async (notification: NotificationData) => {
    ...
    if (!notification.read) {
      try {
        const res = await fetch(`/api/pharmacist/notifications/${notification.id}`, { method: 'PUT' });
        if (res.ok) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-4">
      ...
      {/* Mark all read button (visible even when scrolled) */}
      {unreadCount > 0 && (
        <div className="mb-3">
          <Button
            variant="outline"
